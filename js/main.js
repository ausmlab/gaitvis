...
SpeedChart.prototype.update = function (currentFrame) {
    this.x.domain([(currentFrame - frameWindowSize) * frame2time, (currentFrame) * frame2time]);
...
HeatMap.prototype.initMap = function (data) {
    this.data = data;
};
HeatMap.prototype.updateMap = function (currentFrame) {
    if (heatmapCheckbox.checked) {
        let xBegin = (currentFrame) - frameWindowSize * 3;
        let points = this.data.filter(function (d) {
            return ((xBegin <= d.frame) && (d.frame <= currentFrame));
        }).map(function (d) {
            return {x: Math.round(footstepchart.x(d.cx)), y: Math.round(footstepchart.y(d.cy)), value: 1};
        });
        this.heatmapInstance.setData({max: 1, data: points});
    }
};
