...
//        .x(function(d) { return x((d.rx + d.lx)*0.5); })
//        .y(function(d) { return y((d.ry + d.ly)*0.5); });
var rValueLine = d3.svg.line()
    .interpolate("basis")
    .x(function(d) { return x(d.rx); })
    .y(function(d) { return y(d.ry); });
var lValueLine = d3.svg.line()
    .interpolate("basis")
    .x(function(d) { return x(d.lx); })
    .y(function(d) { return y(d.ly); });
...
SmallGraph.prototype.initGraph = function(anchor) {
...
SmallGraph.prototype.drawAxes = function(xDomain, yDomain, xLabel, yLabel) {
...
SmallGraph.prototype.updateLineGraph = function() {
...
var drawAxes = function() {
    ...
    //x.domain(d3.extent(data, function(d) { return d.rx; })).nice();
    //y.domain(d3.extent(data, function(d) { return d.ry; })).nice();
    x.domain([-10, 10]);
    //x.domain([-2000, 2000]);
    y.domain([-8, 12]);
...
var distanceSpeedLine = d3.svg.line()
    .interpolate("basis")
    .x(function(d) { return speedDistanceGraph.x(d.frame*frame2time); })
    .y(function(d) { return speedDistanceGraph.y(d.speed); });
...
//var opacityLookupTable = [1, 0.8, 0.6, 0.4, 0.2, 0.1];
var opacityLookupTable = [1, 0.0];
//var opacityLookupTable = [1, 0.631272442*.7, 0.15825031*.7, 0.015608801*.7, 0.000598028*.7];

//var dataSource = 'data/YU_VC01_20180620_092549.txt';
//var dataSource = 'data/CAM1_8_UV_BL.txt';
//var dataSource = 'data/YU_VC01_20180620_141810.txt';
var data;
...
var loadData = function(filename) {
    ...
    d3.text(textPath, function (text) {
        ...
        var maxFrame = d3.max(data, function (d) {
            return d.frame;
        })
        slider.max = maxFrame;
        frameCount = maxFrame + 1;

        var groupByID = d3.nest()
            .key(function (d) {
                return d.id;
            })
            .entries(data);
...
//            .attr("class", "dot")
//            .attr("r", 3.5)
//            .attr("cx", function(d) { return x(d.rx); })
//            .attr("cy", function(d) { return y(d.ry); })
...
//                return circleOpacityLookup(d.frame);  });
        ...
        g.append("circle")
            ...
            //.attr("r", function(d) {return Math.sqrt(Math.pow((x(d.rx)-x(d.lx)),2)+Math.pow((y(d.ry)-y(d.ly)),2))*0.5 + 3.5 })
            .attr("r", humanRadius)
            //            .attr("cx", function(d) { return x((d.rx+d.lx)*0.5); })
            //            .attr("cy", function(d) { return y((d.ry+d.ly)*0.5); })
            .attr("cx", function (d) {
                return x(d.cx);
            })
            .attr("cy", function (d) {
...
//        var points = data.filter(function(d) { return (d.frame <= +slider.value); }).map(function(d){
...
var circleOpacityLookup = function (fid) {
    var index = (+slider.value)-fid;
    var opacity = 0.0;
    if ((index>=0) && (index<opacityLookupTable.length))
        opacity = opacityLookupTable[index];
    return opacity;
}
var lineOpacityLookup = function(d) {
    var ex = d3.extent(d.values, function(k) { return k.frame; });
...
heatmapCheckbox.onclick = function () {
...
function handleMouseOver(d) {  // Add interactivity
    ...
    svg.append("text").attr({
        ...
        x: function() { return x(d.cx) - 30; },
        y: function() { return y(d.cy) - 15; }
    })
        .text(function() {
...
var update = function() {
    ...
    //var xEnd = (+slider.value)>frameWindowSize? (+slider.value) : frameWindowSize;
    ...
    //console.log(heatmapCheckbox.checked);
    if (heatmapCheckbox.checked) {
        var xBegin = (+slider.value) - frameWindowSize * 3;
    ...
    lines.selectAll("path")
        .style("opacity", function(d) { return lineOpacityLookup(d);  });

    dots.selectAll(".boundingCircle")
        .filter(function(d) { return (d.frame != +slider.value); })
    ...
    active.each(function(d){
        ...
        active.each(function(e) {
            if(d.id < e.id) {
                //var dist =  Math.sqrt(Math.pow((x(d.cx)-x(e.cx)),2)+Math.pow((y(d.cy)-y(e.cy)),2));
                var dist = euclideanDistance([x(d.cx),y(d.cy)],[x(e.cx),y(e.cy)]);
                //console.log(dist + ">" + humanRadius);
                if (dist<humanRadius*2) {
                    ...
                    //.style("fill", '#ff0000')
                    //.style("opacity", 0.3);
                    //console.log(d.frame+": "+d.id + "-" + e.id +" : " + dist + ">" + humanRadius*2);

                    var sss = speedDistanceGraph.svg.selectAll(".sline")
                        .filter(function(z) {
                            //console.log(z);
                            //  if((d.id == +z.key )||(e.id == +z.key))
                            //      console.log(d.frame+': '+z);
                            return ((d.id == +z.key )||(e.id == +z.key)); })
                    //.style("stroke-width", 10);
                }
            }
            //console.log(d.id + ";" + e.id);
        });
    })
...
function keyDownTextField(e) {
    ...
    if(keyCode==37) {
        slider.value--;
        update();
        //console.log("You hit the left key.");
    } else if(keyCode==39) {
        slider.value++;
        update();
